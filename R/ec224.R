...
taba4 <- mutate(taba4, GD = GF - GC)
# Tabla Acumulada 1raEtapa ORDENAR
taba4 <- taba4[order(-taba4$PTOS, -taba4$GD, -taba4$GF), ]
taba4 <- select(taba4, EQUIPOS, PTOS, GD)
# Tabla Acumulada 1raEtapa Filas ordenadas
rownames(taba4) <- 1:nrow(taba4)
taba4 <- mutate(taba4, J4 = rownames(taba4))
#taba4$J2 <- as.numeric(taba4$J2)
#a2 <- ggplot(taba4, aes(x = EQUIPOS, y = J2, color = EQUIPOS)) 
#a2 +
#  geom_point(size = 2) +

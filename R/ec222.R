...
#Primera Etapa
dat222 <- read_tsv("/home/xut/Documents/udaviz/R/studio/udafutec/data/ec222.txt")
#data1
...
taba2 <- mutate(taba2, GD = GF - GC)
# Tabla Acumulada 1raEtapa ORDENAR
taba2 <- taba2[order(-taba2$PTOS, -taba2$GD, -taba2$GF), ]
taba2 <- select(taba2, EQUIPOS, PTOS, GD)
# Tabla Acumulada 1raEtapa Filas ordenadas
rownames(taba2) <- 1:nrow(taba2)
taba2 <- mutate(taba2, J2 = rownames(taba2))
#taba2$J2 <- as.numeric(taba2$J2)
#a2 <- ggplot(taba2, aes(x = EQUIPOS, y = J2, color = EQUIPOS)) 
#a2 +
#  geom_point(size = 2) +

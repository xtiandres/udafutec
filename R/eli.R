...
# DATA
eli <- read.csv("/home/xut/Documents/udaviz/R/studio/camerica/data/eli.csv")
...
t4 + geom_point(alpha = .6) +
  facet_wrap(~Equipos, ncol = 5) +
  aes(x = fct_inorder(variable)) +
  scale_y_continuous(breaks = seq(0, 100, 10)) +
  geom_text(aes(label = value),
            vjust = -0.5) +
  labs(title = "Eliminatorias Qatar - Conmebol",
       subtitle = "Probabilidad calculada previa 3 últimas fechas ",
       x = "Puestos de Clasificación",
       y = "Porcentajes") +
  theme_minimal() #+
